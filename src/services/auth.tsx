...
export async function loadLocalBanks() {
  let banksList = await Storage.getItem(StorageConstants.BANKS).then(() => getBanks())
...
export const getBanks = async () => {
  ...
  let requestOptions = {
    method: 'GET',
    headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + await Storage.getItem(StorageConstants.TOKEN) }
  };
  try {
...
export const verifyBankDetails = async (accountNumber: any, bankId: any) => {
  ...
  let requestOptions = {
    ...
    headers: {
      ...
      'Authorization': 'Bearer ' + await Storage.getItem(StorageConstants.TOKEN)
    },
    body: JSON.stringify({
...
export const createmandate = async (paymentData: PaymentData | any, signature: any, bankDetails: any) => {
